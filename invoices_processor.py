...
def retrieveAndGenerate(input_prompt, sourceType, model_id, region, document_s3_uri=None, data=None, identifier=None):
    ...
    while True:
        ...
        if 'Contents' in response:
            for obj in response['Contents']:
                ...
                document_uri = "s3://" + bucket_name + "/" + pdf_file_key
                
                # print(document_uri)
                # bedrock_kb.retrieveAndGenerate()
                try:
                    # Download the PDF from S3 and save it to the local folder
                    local_file_path = "./" + pdf_file_key
                    ...
                    no_of_invoices_processed = no_of_invoices_processed + 1

                    results[pdf_file_key] = generated_text
        ...
        if response.get('IsTruncated'):  # Means there are more files
            ...
            break
    return results, no_of_invoices_processed 
...
def main():
    ...
    invoice_data, no_of_invoices_processed = batch_process_s3_bucket_invoices(bucket_name=args.bucket_name, prefix=args.prefix, local_download_folder="invoice")    
